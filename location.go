...
func ParseGeoLocation(longitudeRaw string, latitudeRaw string) (float64, float64, error) {
	...
	if err != nil {
		return 0, 0, NewServerError(400, "longitude", "INVALID_FLOAT", "INVALID_FLOAT", nil)
	}
	...
	if len(errs) > 0 {
		return 0, 0, ServerError{StatusCode: 400, Errors: Errors{Errors: errs}}
	}
...
func LocationParametersFromRequest(req *http.Request, defaultMaxDistance int64) (*LocationParameters, error) {
	longitudeRaw := GetParameterFromURLInRequest(req, "longitude")
	latitudeRaw := GetParameterFromURLInRequest(req, "latitude")
	...
	maxDistanceRaw := GetParameterFromURLInRequest(req, "max_distance")
	minDistanceRaw := GetParameterFromURLInRequest(req, "min_distance")

	if maxDistanceRaw != nil {

		maxDistance, err = strconv.ParseInt(*maxDistanceRaw, 10, 64)
		if err != nil {
			return nil, NewServerError(400, "max_distance", "INVALID_INT", "INVALID_INT", nil)
		}

		errs := validator.ValidateValue(maxDistance, validator.RequiredIntValidators("max_distance", DistanceValidator()))
		if len(errs) > 0 {
			return nil, ServerError{StatusCode: 400, Errors: Errors{Errors: errs}}
		}
	}
	...
	if minDistanceRaw != nil {
		...
		if err != nil {
			return nil, NewServerError(400, "min_distance", "INVALID_INT", "INVALID_INT", nil)
		}
	...
	if minDistance > maxDistance {
		return nil, NewServerError(400, "min_distance", "Min distance more tham max", "MIN_MAX_ERROR", nil)
	}
